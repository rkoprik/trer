...
author: |
  | Rebecca Koprik - extending Joachim Gassen 
...
kable(
  ...
  caption = "\\label{tab:descriptives}Life Expectancy by Income Group",
  format.args = list(decimal.mark = ".", big.mark = ",", 
                     scientific=FALSE),
  booktabs = TRUE, escape = FALSE, linesep = ""
) -> kab_latex
...
latex_tab <- c(
  ...
  "\\footnotetext",
  sprintf(
    "\\item Note: The data is obtained from the World Bank. It presents the average life expectancy by World Bank Income group.", length(unique(smp$country)),
    min(as.numeric(as.character(smp$year))), 
    max(as.numeric(as.character(smp$year)))
  ),
  "\\end{tablenotes}",
...
```{r MyContrib2, echo = FALSE, fig.align="center", fig.cap="\\label{fig:scatter}Life expectancy in years - average for all countries"}
fig_scatter_avg
```

```{r MyContrib3, echo = FALSE, fig.align="center", fig.cap="\\label{fig:scatter}Life expectancy in years - average by income level"}
fig_scatter_level
```
