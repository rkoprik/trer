...
author: "Rebecca Koprik - extending Joachim Gassen"
...
How does income per capita affects life expectancy?
...
```{r MyContrib, results="asis"}
kable(
  tab_expand, digits = c(0, 3, 3, 3, 3, 3, 3, 3), format = "latex",
  caption = "\\label{tab:descriptives}Life expectancy by income level",
  format.args = list(decimal.mark = ".", big.mark = ",", 
                     scientific=FALSE),
  booktabs = TRUE, escape = FALSE, linesep = ""
) -> kab_latex
...
latex_tab <- c(
  ...
  "\\setlength{\\labelsep}{0pt}",
  sprintf(
    "\\item Note: The data is obtained from the World Bank. It presents the average life expectancy by World Bank Income group."),
 "\\end{tablenotes}",
  "\\end{threeparttable}",
  "}",
  "\\end{table}"
...
```{r MyContrib2, echo = FALSE, fig.align="center", fig.cap="\\label{fig:scatter}Life expectancy in years - average for all countries", out.width = "60%"}
fig_scatter_avg
```
### Life expectancy over time

```{r MyContrib3, echo = FALSE, fig.align="center", fig.cap="\\label{fig:scatter}Life expectancy in years - average by income level", out.width = "60%"}
fig_scatter_level
```

By comparing both figures, we can see that the average life expectancy changes drastically by the World Bank Income Group over the years.
